...
Next, we will add an explicit target that does something usefull: we will create a target
called ``readNumber.txt`` for objects of type ``fastq`` that will store the number of paired reads
in the fastq object. This is achieved by replacing the autogenerated one line in the pipeline's 
