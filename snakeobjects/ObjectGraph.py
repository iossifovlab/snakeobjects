...
class ObjectGraph:
    ...
    def createGlblMakefiel(self): 
        ...
        for tp,tpOs in sorted(self.O.items()):
            for ogo in list(tpOs.values()):
                ogo.dir = str(self.baseDir) + "/objects/" + str(ogo.type) + "/" + str(ogo.name)
    ...
    def addObject(self, ob):
        ob.OG = self
        ob.dir = str(self.baseDir) + "/objects/" + str(ob.type) + "/" + str(ob.name)
    ...
    def writeMainSnakefile(self):
        ...
        mf=pdir+'/objects/.snakeobjects/main.snakefile'
        with open(mf, 'w') as f:
            with open(hf, 'r') as g:
                for l in g:
                    f.write(l)
    ...
    def execARGVcommands(OG, ARGV=None):
        ...
        if cmd == "help":
            ...
            OG.writeObjectGraphJson("objects/.snakeobjects/OG.json")
        elif cmd == 'saveAs':
            if len(ARGV) < 3:
